export const PRIORITY_STATIONS = [
  { name: 'גלגלצ', aliases: ['גלגלצ', 'galgalatz', 'glglz'] },
  { name: 'כאן 88', aliases: ['כאן 88', 'kan 88'] },
  { name: 'כאן ב', aliases: ['כאן ב', 'kan bet', 'כאן רשת ב'] },
  { name: 'כאן גימל', aliases: ['כאן גימל', 'kan gimel'] },
  { name: 'גלי צה"ל', aliases: ['גלי צה"ל', 'glz', 'galei zahal'] },
  { name: '103fm', aliases: ['103fm', '103 fm'] },
  { name: 'eco99fm', aliases: ['eco99fm', '99fm', 'eco99 fm'] },
  { name: 'רדיוס 100fm', aliases: ['רדיוס', 'radius', '100fm', '100 fm'] },
  { name: '102fm', aliases: ['102fm', '102 fm', 'רדיו תל אביב'] }
];

export const CORS_PROXY_URL = 'https://corsproxy.io/?';
